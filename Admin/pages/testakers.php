...
<body>
	...
	<div id="page-wrapper">
					<div class="row">
							<div class="col-lg-12">
								<h1 class="page-header">Test Takers</h1>
							</div>
							<!-- /.col-lg-12 -->
						</div>
						<div class="row">
							<div class="col-sm-12 portfolio-item">
								<div class="panel panel-green">
								  ...
								  <div class="panel-heading" style="text-align: center"><h4>LIST OF TEST TAKERS</h4></div>
								  ...
								  <table class="table table-striped table-bordered table-hover">
									...
									<tr>
										...
										<th style="color: #06356f; text-align: center">Full Name</th>
										<th style="color: #06356f; text-align: center">Age</th>
										<th style="color: #06356f; text-align: center">Gender</th>
										
										<th style="color: #06356f; text-align: center">Location</th>
										<th style="color: #06356f; text-align: center">School</th>
										<th style="color: #06356f; text-align: center">Language ID</th>
										<th style="color: #06356f; text-align: center">Result Code</th>
...
<form name="form1" action="" method="post">
									<?php
										   include("../../dbConfig.php");
											  ...
											  $space = " ";
											  $sql = "SELECT * FROM `testtaker` ORDER BY `testtaker_id` DESC LIMIT $page1,7";
											  $result = mysqli_query($db,$sql);
											   while($results = mysqli_fetch_assoc($result)){
											  echo "<tbody>";
											  echo "<tr>";
											  echo "<th><input type='checkbox' name='num[]' value=".$results['testtaker_id']." /></th>";
											  echo "<th>".$results['testtaker_id']."</th>";
											  echo "<th>".$results['firstname']."<i>".$space."</i>".$results['middlename']."<i>".$space."</i>". $results['lastname']."</th>";
											  echo "<th>".$results['age']."</th>";
											  echo "<th>".$results['gender']."</th>";
											  
											  echo "<th>".$results['location']."</th>";
											  echo "<th>".$results['school_name']."</th>";
											  echo "<th>".$results['testLanguage_id']."</th>";
											  
											  echo "<th>".$results['result_code']."</th>";
											  echo "<th>".$results['date_and_time']."</th>";
											  echo "<th><center><a onclick=\"return confirm('Are you sure you want to DELETE this Test Taker?')\" href=delete.php?id=".$results['testtaker_id']." class='btn btn-danger'><i class='glyphicon glyphicon-minus-sign'></i> Delete</center></a></th>";
											  echo "</tr>";
...
<input type="submit" name="submit1" id="button" class="btn btn-danger btn-lg" value="Delete selected" onclick="return confirm('Do you really want to delete all of this?')" ></form>
									<?php
																...
																if(isset($_POST['submit1'])){
																	...
																	while (list ($key,$val) = @each ($box)){
																		mysqli_query($db,"DELETE FROM testtaker WHERE testtaker_id = $val");
																		
																	
																	$sql1 = "INSERT INTO `log` (`id`, `action`, `admin_id`, `table_name`, `table_id`, `date_and_time`) VALUES (NULL, 'delete', '$adminid', 'test takers', '$val', CURRENT_TIMESTAMP)";
												if(mysqli_query($db,$sql1)){
													
												echo"<SCRIPT LANGUAGE='JavaScript'> window.alert('Test Takers Deleted')</SCRIPT>";
																	?>
									...
									<ul class="pagination pagination-lg" style="position: absolute; right: 20px;" >
										...
										<?php
										$res1 = mysqli_query($db,"SELECT * From testtaker");
											...
											$a = ceil($a);
												for($b=1;$b<=$a;$b++){
													echo "<li><a href='testakers.php?page=".$b."'>".$b.'  '."</a></li>";
												}
